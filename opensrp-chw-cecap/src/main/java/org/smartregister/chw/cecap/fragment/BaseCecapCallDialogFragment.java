...
import org.smartregister.chw.cecap.contract.BaseCecapCallDialogContract;
import org.smartregister.chw.cecap.listener.BaseCecapCallWidgetDialogListener;
import org.smartregister.chw.cecap.R;
import org.smartregister.chw.cecap.domain.MemberObject;

public class BaseCecapCallDialogFragment extends DialogFragment implements BaseCecapCallDialogContract.View {
    ...
    public static BaseCecapCallDialogFragment launchDialog(Activity activity, MemberObject MO) {
        BaseCecapCallDialogFragment dialogFragment = BaseCecapCallDialogFragment.newInstance();
    ...
    public static BaseCecapCallDialogFragment newInstance() {
        return new BaseCecapCallDialogFragment();
    }
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container, Bundle savedInstanceState) {
        ViewGroup dialogView = (ViewGroup) inflater.inflate(R.layout.cecap_member_call_widget_dialog_fragment, container, false);
        setUpPosition();
        if (listener == null) {
            listener = new BaseCecapCallWidgetDialogListener(this);
        }
    ...
    private void setCallTitle(ViewGroup rootView, int viewId, final String message) {
        ...
        if (MEMBER_OBJECT.getBaseEntityId().equals(MEMBER_OBJECT.getFamilyHead())) {
            ...
            callTitle.setText(String.format("%s %s", message, getResources().getString(R.string.call_cecap_client)));
        }
    }
    ...
    private void initUI(ViewGroup rootView) {
        if (StringUtils.isNotBlank(MEMBER_OBJECT.getPhoneNumber())) {
            ...
            if (StringUtils.isNotBlank(MEMBER_OBJECT.getFamilyHead())) {
                TextView familyHeadName = rootView.findViewById(R.id.cecap_call_head_name);
                familyHeadName.setText(MEMBER_OBJECT.getFamilyHeadName());
                TextView clientCallHeadPhone = rootView.findViewById(R.id.cecap_call_head_phone);
                ...
                rootView.findViewById(R.id.cecap_layout_family_head).setVisibility(GONE);
            }

            if (!MEMBER_OBJECT.getBaseEntityId().equals(MEMBER_OBJECT.getFamilyHead())) {
                //just a member
                TextView malariaClientNameTextView = rootView.findViewById(R.id.call_cecap_client_name);
                ...
                setCallTitle(rootView, R.id.call_cecap_client_title, "");
                TextView callMalariaClientPhone = rootView.findViewById(R.id.call_cecap_client_phone);
                ...
                rootView.findViewById(R.id.layout_cecap_client).setVisibility(GONE);
            }
        }

        rootView.findViewById(R.id.cecap_call_close).setOnClickListener(listener);
    }
    ...
    @Override
    public void setPendingCallRequest(BaseCecapCallDialogContract.Dialer dialer) {
