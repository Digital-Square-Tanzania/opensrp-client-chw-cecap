...
import static org.smartregister.chw.cecap.util.Constants.EVENT_TYPE.DELETE_EVENT;
import static org.smartregister.chw.cecap.util.Constants.JSON_FORM_EXTRA.DELETE_EVENT_ID;
import static org.smartregister.chw.cecap.util.Constants.JSON_FORM_EXTRA.DELETE_FORM_SUBMISSION_ID;
import static org.smartregister.chw.cecap.util.JsonFormUtils.HOME_VISIT_GROUP;
...
import org.smartregister.chw.cecap.repository.VisitDetailsRepository;
...
public class VisitUtils {
    ...
    public static void processVisits(VisitRepository visitRepository, VisitDetailsRepository visitDetailsRepository, Context context) throws Exception {
        ...
        for (Visit v : visits) {
            ...
            if (v.getVisitType().equalsIgnoreCase(Constants.EVENT_TYPE.CECAP_FOLLOW_UP_VISIT)) {
                try {
                    visitList.add(v);
                } catch (Exception e) {
                    Timber.e(e);
    ...
    public static List<Visit> getVisits(String memberID, String... eventTypes) {
        ...
        List<Visit> visits = (eventTypes != null && eventTypes.length > 0) ? getVisitsOnly(memberID, eventTypes[0]) : getVisitsOnly(memberID, Constants.EVENT_TYPE.CECAP_FOLLOW_UP_VISIT);
    ...
    public static List<Visit> getVisitsOnly(String memberID, String visitName) {
        return new ArrayList<>(CecapLibrary.getInstance().visitRepository().getVisits(memberID, visitName));
    }

    public static List<VisitDetail> getVisitDetailsOnly(String visitID) {
        return CecapLibrary.getInstance().visitDetailsRepository().getVisits(visitID);
    }
    ...
    public static void processVisits(String baseEntityID) throws Exception {
        processVisits(CecapLibrary.getInstance().visitRepository(), CecapLibrary.getInstance().visitDetailsRepository(), baseEntityID);
    }
    ...
    public static void deleteProcessedVisit(String visitID, String baseEntityId) {
        // check if the event
        AllSharedPreferences allSharedPreferences = CecapLibrary.getInstance().context().allSharedPreferences();
        Visit visit = CecapLibrary.getInstance().visitRepository().getVisitByVisitId(visitID);
        if (visit == null || !visit.getProcessed()) return;

        Event processedEvent = CecapDao.getEventByFormSubmissionId(visit.getFormSubmissionId());
