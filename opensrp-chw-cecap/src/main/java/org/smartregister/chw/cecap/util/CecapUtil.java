...
public class CecapUtil {

    public static void processEvent(AllSharedPreferences allSharedPreferences, Event baseEvent) throws Exception {
        if (baseEvent != null) {
            CecapJsonFormUtils.tagEvent(allSharedPreferences, baseEvent);
            JSONObject eventJson = new JSONObject(CecapJsonFormUtils.gson.toJson(baseEvent));
    ...
    public static ECSyncHelper getSyncHelper() {
        return CecapLibrary.getInstance().getEcSyncHelper();
    }

    public static ClientProcessorForJava getClientProcessorForJava() {
        return CecapLibrary.getInstance().getClientProcessorForJava();
    }
    ...
    public static boolean launchDialer(final Activity activity, final BaseCecapCallDialogContract.View callView, final String phoneNumber) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {

            // set a pending call execution request
            if (callView != null) {
                callView.setPendingCallRequest(() -> CecapUtil.launchDialer(activity, callView, phoneNumber));
            }
    ...
    public static void saveFormEvent(final String jsonString) throws Exception {
        ...
        Event baseEvent = CecapJsonFormUtils.processJsonForm(allSharedPreferences, jsonString);
        CecapUtil.processEvent(allSharedPreferences, baseEvent);
    }
    ...
    public static class CloseCecapMemberFromRegister extends AsyncTask<Void, Void, Void> {
        private final String baseEntityId;

        public CloseCecapMemberFromRegister(String baseEntityId) {
            this.baseEntityId = baseEntityId;
        }

        @Override
        protected Void doInBackground(Void... voids) {
            CecapDao.closeCecapMemberFromRegister(baseEntityId);
            return null;
        }
